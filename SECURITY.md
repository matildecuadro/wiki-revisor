...
La clave se guarda mediante `GM_setValue`, en el almacenamiento propio de la extensión Tampermonkey, aislado por script — no es accesible desde el JavaScript de la propia página de Wikipedia ni desde otros userscripts que no compartan namespace. Este aislamiento es real y es la primera barrera de protección.
...
- Ningún cambio en el propio userscript puede eliminar este límite por completo: es una característica estructural de guardar secretos del lado del cliente en un navegador, no un descuido específico de esta implementación.
...
Esta es la medida con más impacto real y la que se recomienda encarecidamente:
...
*(Pendiente: definir canal de contacto para reportes — ver README, sección Contribuir)*
